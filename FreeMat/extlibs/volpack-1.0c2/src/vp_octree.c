...
static vpResult CreatePyramid ANSI_ARGS((vpContext *vpc,
...
static void DescendPyramid ANSI_ARGS((vpContext *vpc,
    ...
    int *octree_offset));
static vpResult Compute1DSummedAreaTable ANSI_ARGS((vpContext *vpc));
static vpResult Compute2DSummedAreaTable ANSI_ARGS((vpContext *vpc));
...
int base_node_size;	/* controls level of detail of smallest nodes */
{
    ...
    return_val = CreatePyramid(vpc, mm_pyramid);
    if (return_val != VP_OK) return return_val;
...
vpContext *vpc;
{
    ...
    case 1:
	return Compute1DSummedAreaTable(vpc);
	break;
    case 2:
	return Compute2DSummedAreaTable(vpc);
	break;
    default:
...
int max_level;
{
    ...
    /* classify the octree */
    result = VPComputeSummedAreaTable(vpc);
    if (result != VP_OK) return(result);
    VPClassifyOctree(vpc);
