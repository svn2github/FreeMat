...
Mat_Create(char *matname,char *hdr_str)
{
    ...
    err = fwrite(endianchar,2,1,mat->fp);
...
Mat_VarGetCell(matvar_t *matvar,int index)
{
    ...
    if ( matvar == NULL || matvar->fp == NULL || matvar->fp->version == MAT_V4 )
        return NULL;
    ...
    int *stride,int *edge)
{
    ...
    if ( (matvar == NULL) || (start == NULL) || (stride == NULL) ||  
        ...
        return NULL;
    } else if ( matvar->fp == NULL || matvar->fp->version == MAT_V4 ) {
        return NULL;
    }
...
Mat_VarGetStructField(matvar_t *matvar,void *name_or_index,int opt,int index)
{
    ...
    if ( matvar->fp == NULL || matvar->fp->version == MAT_V4 ) {
        return NULL;
    }
    nmemb = 1;
    for ( i = 0; i < matvar->rank; i++ )
...
Mat_VarPrint( matvar_t *matvar, int printdata )
{
    if ( matvar == NULL || matvar->fp == NULL )
        return;
    else if ( matvar->fp->version != MAT_V4 )
