...
test_write( void )
{
    ...
    if ( mat ) {
        ...
        dims[0] = 1;
        dims[1] = strlen(str);
        matvar = Mat_VarCreate((const char *)"str",MAT_C_CHAR,MAT_T_INT8,2,
                               dims,str,0);
        Mat_VarWrite( mat, matvar, 0);
        Mat_VarFree(matvar);
        Mat_Close(mat);
...
test_readvar4(const char *inputfile, const char *var)
{
    ...
    mat = Mat_Open(inputfile,MAT_ACC_RDONLY | MAT_V4);
    if ( mat ) {
        matvar = Mat_VarRead(mat,var);
        if ( matvar == NULL ) {
            err = 1;
...
test_write_null(void)
{
    int       err = 0;
    mat_t    *mat;
    matvar_t *matvar,*struct_matvar,*cell_matvar;
...
test_get_struct_field( const char *file )
{
    ...
    int index = 1, subs[2], err = 0;

    mat = Mat_Open(file,MAT_ACC_RDONLY);
    if ( mat ) {
        matvar = Mat_VarRead(mat,"structure");
        if ( matvar ) {
            field = Mat_VarGetStructField(matvar,"name",BY_NAME,0);
            ...
            subs[0] = 2;
            subs[1] = 2;
            index = Mat_CalcSingleSubscript(matvar->rank,matvar->dims,subs);
            field = Mat_VarGetStructField(matvar,"name",BY_NAME,index);
            Mat_VarPrint( field, 0);
            index = 2;
            field = Mat_VarGetStructField(matvar,&index,BY_INDEX,index);
            Mat_VarPrint( field, 0);
...
test_readslab(const char *file, const char *var)
{
   ...
   matvar_t *matvar;
    ...
    mat = Mat_Open(file,MAT_ACC_RDONLY);
    if ( mat ) {
        matvar = Mat_VarReadInfo(mat,(const char *)var);
        if ( matvar != NULL ) {
...
test_readslab4(const char *file, const char *var)
{
   ...
   matvar_t *matvar;
    ...
    mat = Mat_Open((const char *)file,MAT_ACC_RDONLY | MAT_V4);
    if ( mat ) {
