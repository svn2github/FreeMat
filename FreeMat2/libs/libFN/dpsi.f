...
C   machine-dependent FUNPACK program DPSI(X), but cannot match that
...
C            arguments for DPSI.  If rounding is necessary, round this
C            value down.
...
C            1+eps > 1. 
C   XLARGE = argument beyond which DPSI(X) may be represented by
...
C----------------------------------------------------------------------
      INTEGER I,N,NQ
      DOUBLE PRECISION
     1   AUG,CONV,DEN,DPSI,FOUR,FOURTH,HALF,ONE,P1,P2,PIOV4,Q1,Q2,
