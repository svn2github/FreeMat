/* ../rkbesl.f -- translated by f2c (version 20031025).
...
/* Subroutine */ int rkbesl_(integer *x, integer *alpha, integer *nb, integer 
	*ize, integer *bk, integer *ncalc)
{
    ...
    double log(doublereal), exp(doublereal), sinh(doublereal), sqrt(
	    doublereal), r_int(real *);

    /* Local variables */
    static real a;
    static integer c__, d__, i__, j, k, m, p[8], q[7], r__[5], s[4], t[6], d1,
	     d2, d3, f0, f1, f2, p0, q0, t1, t2, dm, ex, bk1, bk2, one, enu, 
	    eps, two, half, iend, estf[7], xinf, estm[6], x2by4, four, xmin, 
	    xmax, zero, twox, blpha, itemp, ratio, wminf, tinyx, twonu, 
	    mplus1, sqxmin;
...
/* D    DATA HALF,ONE,TWO,ZERO/0.5D0,1.0D0,2.0D0,0.0D0/ */
/* D    DATA FOUR,TINYX/4.0D0,1.0D-10/ */
/* D    DATA A/ 0.11593151565841244881D0/,D/0.797884560802865364D0/ */
/* --------------------------------------------------------------------- */
/*  Machine dependent parameters */
/* --------------------------------------------------------------------- */
/* S    DATA EPS/1.19E-7/,SQXMIN/1.08E-19/,XINF/3.40E+38/ */
/* S    DATA XMIN/1.18E-38/,XMAX/85.337E0/ */
/* D    DATA EPS/2.22D-16/,SQXMIN/1.49D-154/,XINF/1.79D+308/ */
/* D    DATA XMIN/2.23D-308/,XMAX/705.342D0/ */
/* --------------------------------------------------------------------- */
...
/* S   1          8.45096E1, 2.0E1/ */
/* D    DATA P/ 0.805629875690432845D00,    0.204045500205365151D02, */
/* D   1        0.157705605106676174D03,    0.536671116469207504D03, */
/* D   2        0.900382759291288778D03,    0.730923886650660393D03, */
/* D   3        0.229299301509425145D03,    0.822467033424113231D00/ */
/* D    DATA Q/ 0.294601986247850434D02,    0.277577868510221208D03, */
/* D   1        0.120670325591027438D04,    0.276291444159791519D04, */
/* D   2        0.344374050506564618D04,    0.221063190113378647D04, */
/* D   3        0.572267338359892221D03/ */
/* D    DATA R/-0.48672575865218401848D+0,  0.13079485869097804016D+2, */
/* D   1       -0.10196490580880537526D+3,  0.34765409106507813131D+3, */
/* D   2        0.34958981245219347820D-3/ */
/* D    DATA S/-0.25579105509976461286D+2,  0.21257260432226544008D+3, */
/* D   1       -0.61069018684944109624D+3,  0.42269668805777760407D+3/ */
/* D    DATA T/ 0.16125990452916363814D-9, 0.25051878502858255354D-7, */
/* D   1        0.27557319615147964774D-5, 0.19841269840928373686D-3, */
/* D   2        0.83333333333334751799D-2, 0.16666666666666666446D+0/ */
/* D    DATA ESTM/5.20583D1, 5.7607D0, 2.7782D0, 1.44303D1, 1.853004D2, */
/* D   1          9.3715D0/ */
/* D    DATA ESTF/4.18341D1, 7.1075D0, 6.4306D0, 4.25110D1, 1.35633D0, */
/* D   1          8.45096D1, 2.0D1/ */
/* --------------------------------------------------------------------- */
...
L100:
		...
		bk1 += t1;
		bk2 += t2;
		if ((i__1 = t1 / (f1 + bk1), abs(i__1)) > eps || (i__2 = t2 / 
			(f2 + bk2), abs(i__2)) > eps) {
		    goto L100;
		}
		bk1 = f1 + bk1;
...
/* L140: */
		...
		r__1 = (real) ex;
		p0 = exp(c__ * (a + c__ * (p[7] - c__ * d1 / t1) - log(r__1)))
			 / ex;
...
/* L160: */
		...
		r__1 = (real) ex;
		bk1 = one / ((d__ + d__ * blpha) * sqrt(r__1));
		if (*ize == 1) {
		    r__1 = (real) (-ex);
		    bk1 *= exp(r__1);
		}
		wminf = estf[4] * (ex - (i__1 = ex - estf[6], abs(i__1))) + 
			estf[5];
	    }
/* -------------------------------------------------------------------- */
