/* ribesl.f -- translated by f2c (version 20031025).
...
/* Subroutine */ int ribesl_(integer *x, real *alpha, integer *nb, integer *
	ize, integer *b, integer *ncalc)
{
    ...
    /* Local variables */
    static integer k, l, n, p, em, en, one, sum, two, half, nend;
    extern integer func_();
    static integer magx, pold, nsig, nbmx;
    extern integer conv_(integer *);
    static integer test, zero, empal, halfx, ensig, tempa, tempb, tempc, 
	    enten, psave, plast, const__, tover, emp2al, xlarge, enmten, 
	    exparg, psavel, rtnsig, nstart;
...
/* D    DATA ONE,TWO,ZERO,HALF,CONST/1.0D0,2.0D0,0.0D0,0.5D0,1.585D0/ */
/* ------------------------------------------------------------------- */
...
/* D    DATA NSIG,XLARGE,EXPARG /16,1.0D4,709.0D0/ */
/* D    DATA ENTEN,ENSIG,RTNSIG/1.0D308,1.0D16,1.0D-4/ */
/* D    DATA ENMTEN/8.9D-308/ */
/* ------------------------------------------------------------------- */
...
/* ------------------------------------------------------------------- */
    ...
    /* Function Body */
    if (*nb > 0 && *x >= zero && *alpha >= (real) zero && *alpha < (real) one 
...
L230:
		...
		sum = sum * func_(&r__1) * pow_dd(&d__1, &d__2);
	    }
	    if (*ize == 1) {
		r__1 = (real) (-(*x));
		sum *= exp(r__1);
...
/* ------------------------------------------------------------------- */
		...
		d__2 = (doublereal) (*alpha);
		tempa = pow_dd(&d__1, &d__2) / func_(&empal);
	    }
	    if (*ize == 2) {
		r__1 = (real) (-(*x));
		tempa *= exp(r__1);
