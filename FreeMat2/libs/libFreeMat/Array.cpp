...
if (source < lowerBound)\
  ...
  case FM_FLOAT:
	...
	caseMacro(FM_UINT32,uint32,keepBounds(uint32, qp[i], sp[i], 0, 4294967295));
	caseMacro(FM_INT32,int32,keepBounds(int32, qp[i], sp[i], -2147483648, 2147483647));
	caseMacro(FM_UINT64,uint64,keepBounds(uint64, qp[i], sp[i], 0, (unsigned long)18446744073709551615));
	caseMacro(FM_INT64,int64,keepBounds(int64, qp[i], sp[i], (long)-922337203685477580, (long)9223372036854775807));
	caseMacro(FM_DOUBLE,double,qp[i] = (double) sp[i]);
	caseMacro(FM_COMPLEX,float,qp[i<<1] = (float) sp[i]);
	caseMacro(FM_DCOMPLEX,double,qp[i<<1] = (double) sp[i]);	
      }
  ...
  case FM_COMPLEX:
	...
	caseMacro(FM_UINT16,uint16,keepBounds(uint16, qp[i], sp[i<<1], 0, 65535));
	caseMacro(FM_INT16,int16,keepBounds(int16, qp[i], sp[i<<1], -32768, 32767));
	caseMacro(FM_UINT32,uint32,keepBounds(uint32, qp[i], sp[i<<1], 0, 4294967295));
	caseMacro(FM_INT32,int32,keepBounds(int32, qp[i], sp[i<<1], -2147483648, 2147483647));
	caseMacro(FM_UINT64,uint64,keepBounds(uint64, qp[i], sp[i<<1], 0, (unsigned long)18446744073709551615));
	caseMacro(FM_INT64,int64,keepBounds(int64, qp[i], sp[i<<1], (long)-922337203685477580, (long)9223372036854775807));
