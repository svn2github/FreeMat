...
%	d = function('diff,forw', x, offsets, [lastdim]);
%		d = C * x
...
%
%	d = function('diff,forw2', ...);
%		d = C.^2 * x
%		like 'diff,forw' but with squared c_kj values.
%
%	x = function('diff,back', d, offsets, [lastdim]);
%		offsets is int32 array such as [-1 -nx -nx-1 -nx+1]
%		x = C' * d
%		output x is of size [length(d(:))/length(offsets)]
%
%	x = function('diff,back2', wk, offsets, [lastdim]);
%		like diff,back but with squared c_kj values.
...
if streq(arg, 'diff,forw')
	...
	penalty_diffs_forw(out, x, nn, offsets, length(offsets), nrep);

elseif streq(arg, 'diff,forw2')
	x = varargin{1};
	offsets = varargin{2};
	if length(varargin) == 3, 
	  repdim = varargin{3}; 
	...
	outsize = [xsize(1:repdim),length(offsets),xsize((repdim+1):end)];
        ...
        nn = max(1,prod(xsize(1:repdim)));
	nrep = max(1,prod(xsize((repdim+1):end)));
	penalty_diffs_forw2(out, x, nn, offsets, length(offsets), nrep);

elseif streq(arg, 'diff,back')
	d = varargin{1};
	offsets = varargin{2};
	if length(varargin) == 3, 
	  repdim = varargin{3}; 
	...
	penalty_diffs_back(out, d, nn, offsets, length(offsets), nrep);

elseif streq(arg, 'diff,back2')
	d = varargin{1};
	offsets = varargin{2};
	if length(varargin) == 3, 
	  repdim = varargin{3}; 
	...
	outsize = [dsize(1:repdim),dsize((repdim+2):end)];
        ...
        nn = max(1,prod(dsize(1:repdim)));
	nrep = max(1,prod(dsize((repdim+2):end)));
	penalty_diffs_back2(out, d, nn, offsets, length(offsets), nrep);
...
myimport(libname, 'penalty_diffs_forw', 'void', type1f);
myimport(libname, 'penalty_diffs_back', 'void', type1b);
myimport(libname, 'penalty_diffs_forw2', 'void', type1f);
myimport(libname, 'penalty_diffs_back2', 'void', type1b);
