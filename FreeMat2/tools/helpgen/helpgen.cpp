...
void TestWriter::OutputText(QString text) {
  ...
  myStream << QString("function success = test_%1\n").arg(num);
  myStream << "\n";
...
void WriteMainTestRoutine() {
  QFile file(sourcepath+"/help/test/run_tests.m");
  if (!file.open(QFile::WriteOnly))
    Halt("Unable to open "+sourcepath+"/help/test/run_tests.m for output");
  QTextStream o(&file);
  o << "%\n";
  ...
  o << "     if (~success) failed = [failed,testlist(i)]; end;\n";
  o << "     printf('Completed test %s\\n',testlist{i}(1:end-2));\n";
  o << "     close all\n";
  o << "   end\n";
  QFile file2(sourcepath+"/help/test/test_exact.m");
  QTextStream q(&file2);
