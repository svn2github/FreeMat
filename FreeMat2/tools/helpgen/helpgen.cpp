...
/*
 * NOTE - this should be transitioned to a .m file
 */
...
Interpreter* GetInterpreter(QStringList cmds) {
  ...
  m_interp = twalk;
  FunctionDef *doCLI;
  if (!twalk->lookupFunction("docli",doCLI))
    return 0;
  twalk->setThreadFunc(doCLI,ArrayVector());
  return twalk;
}
...
class HelpWriter {
 public:
  ...
  virtual void DoComputeBlock(QStringList cmdList, int errorsExpected) {};
};
...
void GroupWriter::DoComputeBlock(QStringList cmds, int errorsExpected) {
  for (unsigned i=0;i<cast.size();i++) 
    cast[i]->DoComputeBlock(cmds,errorsExpected);
}
...
class BBTestWriter : public HelpWriter {
  QMultiMap<QString, QStringList> sectables;
  bool verbatim;
  bool ignore;
  QString module;
 ...
 public:
  ...
  void OutputText(QString text);
  void EndModule();
  void DoComputeBlock(QStringList cmdList, int errorsExpected);
};
...
void BBTestWriter::BeginModule(QString modname, QString moddesc, QString secname) {
  module = modname;
  ...
  *stream << "% Regression test function (black box) for " << 
    Interpreter::getVersionString().c_str() << "\n";
  *stream << "% This function is autogenerated by helpgen.\n";
  *stream << QString("function bbtest_success = bbtest_%1\n").arg(modname.toLower());
  *stream << "  bbtest_success = 1;\n";
}

BBTestWriter::BBTestWriter() {
  file = NULL;
  stream = NULL;
}

void BBTestWriter::BeginGroup(QString groupname) {
}

void BBTestWriter::OutputText(QString text) {
}
...
void TestWriter::OutputText(QString text) {
  ...
  if (test_pattern.indexIn(text) < 0)
    Halt("Bad line: " + text);
  QString filename(sourcepath+QString("/help/test/wbtest_%1_%2.m").arg(module.toLower()).arg(num));
  ...
  myStream << QString("function success = wbtest_%1_%2\n").arg(module.toLower()).arg(num);
  myStream << "\n";
...
QString EvaluateCommands(QStringList cmds, int expectedCount, QString modulename, QString file) {
  ...
  while (!twalk->isFinished())
    qApp->processEvents();
  if (!twalk->wait(50000))
    Halt("Error: timeout waiting for interpreter thread to return");
...
void ProcessFile(QFileInfo fileinfo, HelpWriter *out) {
  ...
  Scope *m_scope = new Scope("global");
  context = new Context(m_scope);
  ...
  if (fileinfo.suffix() == "cpp" | fileinfo.suffix() == "m") {
	...
	if (TestMatch(docblock_pattern,line)) {
	  ...
	  while (!fstr.atEnd() && !TestMatch(docblock_pattern,line)) {
		...
		out->DoComputeBlock(cmdlist,ErrorsExpected);
...
void WriteMainTestRoutine() {
  QString o;
  o += "%\n";
  o += "% Function run_tests\n";
  o += "%\n";
  o += "%  Runs the complete set of tests for FreeMat\n";
  o += "%  This file is autogenerated by helpgen (the help compiler)\n";
  o += "%  Any changes will be lost!\n";
  o += "function failed = run_tests\n";
  o += "   testlist = system('ls bbtest_*.m');\n";
  o += "   testlist = [testlist;system('ls wbtest_*.m')];\n";
  ...
  o += "     if (isempty(success) || ~success) failed = [failed,testlist(i)]; end;\n";
  o += "     printf('Completed test %s\\n',testlist{i}(1:end-2));\n";
  o += "     close all\n";
  o += "   end\n";
  o += "   printf('****************************************\\n');\n";
  o += "   printf('*  Test Summary                        *\\n');\n";
  o += "   printf('****************************************\\n');\n";
  o += "   printf('\\n');\n";
  o += "   printf('Number of tests: %d\\n',numel(testlist));\n";
  o += "   printf('Failed tests: %d\\n',numel(failed));\n";
  o += "   for i=1:numel(failed)\n";
  o += "      printf('   Test %s failed\\n',failed{i});\n";
  o += "   end\n";
...
void usage() {
  std::cerr << "Usage: helpgen <path_to_source>\n";
  exit(1);
}
